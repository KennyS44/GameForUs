// PROPOSED layout, drawn but not yet played.
//
// This is the redesign as I understood it, kept beside the live map rather
// than replacing it: tools/floorplan.mjs draws it, the game still loads
// apartment.js. Once the plans are approved this file becomes the map.
//
// Axes as always: +X east, +Z south, +Y up. Ground floor y = 0, upper y = 3.3.
//
// What changed, in the order it was asked for:
...
const shell = [
  ...
  ...wall(-16, -18, -5, -18, M.concrete, { thickness: 0.3, height: RAIL, tag: 'parapet' }),
  ...
  ...slab(-8, -18.2, 13.6, -17.1), // north strip, east of the terrace arm
  ...slab(-8, -17.1, 13.75, -15.6), // ...split around the hole in the store
  ...slab(15.25, -17.1, 16.2, -15.6),
  ...slab(-8, -15.6, 16.2, 6.2), // everything south of the hole
  ...slab(13.6, -18.2, 16.2, -17.1),
  ...slab(-8, -10.5, -5, -7.4), // the terrace's east arm meets its south arm
  ...slab(-16.2, -10.5, -8, -7.4), // south arm of the terrace
  ...slab(-8, -18.2, -5, -10.5), // east arm of the terrace
  ...slab(-16.2, -7.4, 13.6, 6.2), // the rest of the upper storey
...
const courtStair = [
  ...
  box(-12.4, 0, -13.7, -9.2, F2 / 2, -12.9, M.floor), // half-landing
  ...flight({ x1: -10.7, x2: -9.2, zBottom: -13.7, dir: 1, steps: 8, from: F2 / 2, rise: F2 / 2 }),
  ...slab(-10.7, -11.3, -9.2, -10.5), // top step meets the terrace's south arm
  // The flights are open on their long sides; a low parapet keeps you on them.
  ...wall(-12.55, -13.7, -12.55, -11.0, M.concrete, { thickness: 0.15, height: RAIL, tag: 'parapet' }),
  ...wall(-9.05, -13.7, -9.05, -10.5, M.concrete, { thickness: 0.15, height: RAIL, tag: 'parapet' }),
];
...
const groundWalls = [
  ...
  ...wall(-16, 1.5, 3, 1.5, M.drywall, {
    gaps: [
      { at: -15, width: 1.0 }, // cinema ↔ cloakroom, west end
      { at: -4, width: 1.0 }, // cinema ↔ cloakroom, east end
...
const upperWalls = [
  ...
  ...wall(-16, -10.5, -8, -10.5, M.glass, { base: F2, thickness: 0.1, height: RAIL, tag: 'railing' }),
...
const doors = [
  ...
  { id: 'cinema-cloak-w', pos: { x: -15, z: 1.5 }, axis: 'x', width: 1.0, hinge: -1, swing: 1, material: M.wood },
  { id: 'cinema-cloak-e', pos: { x: -4, z: 1.5 }, axis: 'x', width: 1.0, hinge: 1, swing: 1, material: M.wood },
  // Kicked in before the round even starts, and blocked by the wardrobe.
  { id: 'hall-gym', pos: { x: 3, z: -2 }, axis: 'z', width: 1.0, hinge: -1, swing: 1, material: M.wood, forced: true },
...
const L2 = F2 + 2.65;
const lights = [
  { id: 'court-w', pos: { x: -12, y: L2 + 1.4, z: -14 }, radius: 14, color: 0x9fb4c8, intensity: 2.2, mount: 'post', base: 0 },
  ...
  { id: 'master', pos: { x: -10, y: L2, z: -2 }, radius: 9, color: 0xffc890, intensity: 1.2 },
...
const rooms = [
  ...
  { id: 'spine-0', name: 'Коридор', floor: 0, min: { x: -16, z: -7.4 }, max: { x: 13.6, z: -4.6 } },
...
export const PENTHOUSE_V2 = {
  id: 'penthouse-v2',
  name: 'Пентхаус (проект)',
  draft: true,
  bounds: { min: { x: -16.2, y: 0, z: -18.2 }, max: { x: 16.2, y: ROOF + 0.3, z: 9.7 } },
  upperFloorY: F2,
  geometry: [...shell, ...courtStair, ...groundWalls, ...upperWalls, ...eastStair, ...blockers],
  rooms,
  doors,
  lights,
  holes,
